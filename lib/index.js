...
//   /dsh-whale/whale.png  — the widget image (custom PNG → bundled PNG → default SVG)
...
// Bundled whale image (lib/assets/DSniang02.png): shown whenever imagePath is
// not configured or the configured path is unreadable, so every install shows
// the whale PNG with zero configuration.
const BUNDLED_IMAGE_PATH = fileURLToPath(new URL("./assets/DSniang02.png", import.meta.url));
let bundledImageBytes = null; // null = not loaded yet (re-tried on each request)
...
function apply(ctx, config) {
  ...
  // Image route: custom PNG when imagePath is set and readable; otherwise the
  // bundled DSniang02.png; the inline SVG only as the final fallback.
    ...
    handler: async (req, res) => {
      ...
      if (imagePath !== "" && fs !== undefined) {
        try {
          ...
          bytes = await fs.readBytes(target, undefined, 8 * 1024 * 1024);
        } catch (e) {
          bytes = null; /* configured path unreadable → fall through */
        }
      }
      if (bytes === null && fs !== undefined) {
        if (bundledImageBytes === null) {
          try {
            const target = await fs.resolve(BUNDLED_IMAGE_PATH);
            bundledImageBytes = await fs.readBytes(target, undefined, 8 * 1024 * 1024);
          } catch (e) {
            bundledImageBytes = null; /* bundled asset missing → SVG fallback */
          }
        }
        bytes = bundledImageBytes;
      }
      if (bytes !== null) {
        res.writeHead(200, {
